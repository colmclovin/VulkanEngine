...
void VulkanEngine::run() {
	...
	// Initialize game after Vulkan is ready
	m_Game = std::make_unique<Game>(this, window);
	m_Game->Init();

	mainLoop();

	// Cleanup game before Vulkan
	m_Game->Shutdown();
	m_Game.reset();
...
void VulkanEngine::createLogicalDevice() {
	...
	// Device layers are deprecated - validation layers should only be set on the instance
	createInfo.enabledLayerCount = 0;
...
void VulkanEngine::createGraphicsPipeline() {
    ...
    // Vertex input binding (QuadVertex structure)
    VkVertexInputBindingDescription bindingDescription{};
    bindingDescription.binding = 0;
    bindingDescription.stride = sizeof(float) * 10; // 3 (pos) + 4 (color) + 2 (texCoord) + 1 (texIndex)
...
void VulkanEngine::drawFrame() {
    ...
    // Only draw if we have data from Renderer2D
    if (batchIndexCount > 0 && vertexBuffer != VK_NULL_HANDLE && indexBuffer != VK_NULL_HANDLE) {
        // Push view-projection matrix
        vkCmdPushConstants(commandBuffers[imageIndex], pipelineLayout,
            VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(viewProjectionMatrix), viewProjectionMatrix);
        ...
        // Draw indexed
        vkCmdDrawIndexed(commandBuffers[imageIndex], batchIndexCount, 1, 0, 0, 0);
    }
...
void VulkanEngine::createVertexBuffer(const void* vertexData, VkDeviceSize size) {
...
void VulkanEngine::createIndexBuffer(const void* indexData, VkDeviceSize size) {
    currentIndexBufferSize = size;
...
void VulkanEngine::updateVertexBuffer(const void* vertexData, VkDeviceSize size) {
    // If buffer needs to grow, recreate it
    if (size > currentVertexBufferSize) {
        if (vertexBuffer != VK_NULL_HANDLE) {
            vkDestroyBuffer(device, vertexBuffer, nullptr);
            vkFreeMemory(device, vertexBufferMemory, nullptr);
        }
        createVertexBuffer(vertexData, size);
    } else {
...
void VulkanEngine::updateIndexBuffer(const void* indexData, VkDeviceSize size) {
    // If buffer needs to grow, recreate it
    if (size > currentIndexBufferSize) {
        if (indexBuffer != VK_NULL_HANDLE) {
            vkDestroyBuffer(device, indexBuffer, nullptr);
            vkFreeMemory(device, indexBufferMemory, nullptr);
        }
        createIndexBuffer(indexData, size);
    } else {
        // Otherwise, just update the existing buffer
        VkBuffer stagingBuffer;
        VkDeviceMemory stagingBufferMemory;
...
void VulkanEngine::SubmitRenderBatch(const RenderBatch& batch) {
    // Update buffers with batch data
    VkDeviceSize vertexSize = batch.vertexCount * sizeof(float) * 10; // QuadVertex: 3+4+2+1 = 10 floats
    VkDeviceSize indexSize = batch.indexCount * sizeof(uint32_t);

    updateVertexBuffer(batch.vertices, vertexSize);
    updateIndexBuffer(batch.indices, indexSize);

    // Store batch info for rendering
    batchIndexCount = batch.indexCount;
    memcpy(viewProjectionMatrix, batch.viewProjectionMatrix, sizeof(viewProjectionMatrix));
}
...
void VulkanEngine::MouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
    ...
    if (engine && engine->m_Game) {
        engine->m_Game->OnMouseButton(button, action, mods);
    }
}
...
void VulkanEngine::CursorPositionCallback(GLFWwindow* window, double xpos, double ypos) {
    VulkanEngine* engine = static_cast<VulkanEngine*>(glfwGetWindowUserPointer(window));
    if (engine && engine->m_Game) {
        engine->m_Game->OnMouseMove(xpos, ypos);
    }
}
...
void VulkanEngine::ScrollCallback(GLFWwindow* window, double xoffset, double yoffset) {
    VulkanEngine* engine = static_cast<VulkanEngine*>(glfwGetWindowUserPointer(window));
    if (engine && engine->m_Game) {
        engine->m_Game->OnScroll(xoffset, yoffset);
    }
}
